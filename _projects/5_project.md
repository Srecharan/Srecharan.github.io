...
description: A comprehensive comparison of three leading generative model architectures for image synthesis
...
This project explores three fundamentally different approaches to deep generative modeling for image synthesis, focusing on a comparative analysis of Generative Adversarial Networks (GANs), Variational Autoencoders (VAEs), and Diffusion Models. Each architecture was implemented and evaluated on the challenging CUB-200-2011 bird dataset, which requires models to capture subtle visual features and details. The implementation includes custom architectures, loss functions, and training strategies, with rigorous evaluation using the Fréchet Inception Distance (FID) metric to quantitatively assess the quality of generated images.

<div style="text-align: center;">
    <img src="/assets/img/project-5/gan_figure.png" alt="GAN Architecture" style="width: 100%; max-width: 800px;">
    <p><em>Overview of the GAN architecture showing generator and discriminator networks with ResBlock components</em></p>
</div>
...
My exploration began with implementing and comparing three distinct GAN variants, each with progressively improved stability and performance:
...
I implemented three different GAN variants, each with its unique loss function and training dynamics:

- **Vanilla GAN**: The original formulation using Binary Cross-Entropy loss. While conceptually elegant, it suffered from significant training instability and mode collapse, achieving an FID score of 104.62.

- **Least Squares GAN (LSGAN)**: Replaced the sigmoid cross-entropy with a least-squares loss, minimizing the squared difference between the discriminator's output and target labels. This modification produced more stable training and improved sample diversity, with an FID score of 52.48.
...
- **Wasserstein GAN with Gradient Penalty (WGAN-GP)**: The most sophisticated implementation, utilizing the Wasserstein distance with gradient penalty for enforcing the 1-Lipschitz constraint. The discriminator (now a "critic") estimates the Wasserstein distance between real and generated distributions. The gradient penalty term was implemented by sampling interpolated points between real and fake images and penalizing deviations of the gradient norm from 1. This approach demonstrated the most stable training and generated the highest quality images, with an FID score of 33.07.
...
The second phase focused on Variational Autoencoders, exploring their unique ability to learn structured latent representations while balancing reconstruction quality and sampling capability.

#### 3.1 VAE Architecture

I implemented a standard VAE architecture with several key components:
...
<div style="text-align: center;">
    <img src="/assets/img/project-5/vae_figure.png" alt="VAE Architecture" style="width: 100%; max-width: 800px;">
...
I conducted experiments with different latent space dimensions to explore the trade-offs:
...
To optimize the balance between reconstruction accuracy and latent space regularity, I implemented:

- **β Parameter Control**: Investigated different values of β (0.8, 1.0, 1.2) which controls the weight of the KL divergence term in the loss function
- **β-Annealing**: Implemented a linear annealing schedule that gradually increased β from 0 to the target value (0.8) over 20 epochs
...
The final phase focused on Diffusion Models, specifically investigating sampling strategies for a pre-trained model.
...
<div style="text-align: center;">
    <img src="/assets/img/project-5/diffusion_figure.png" alt="Diffusion Model" style="width: 100%; max-width: 800px;">
    <p><em>Diffusion model architecture showing forward noising process and learned reverse denoising process</em></p>
</div>

#### 4.2 Sampling Strategies

I implemented and compared two sampling approaches:
...
- Implementation of three different GAN architectures with custom ResBlock components and loss functions
- Development of the gradient penalty mechanism for WGAN-GP to enforce the 1-Lipschitz constraint
- Exploration of VAE latent space dimensions and implementation of β-annealing for improved sample quality
- Implementation of both DDPM and DDIM sampling strategies for diffusion models
- Comprehensive comparative analysis of different generative architectures on the same dataset
...
- **Evaluation Metrics**: Fréchet Inception Distance (FID), Reconstruction Loss

---

### 8. Project Repository

[GenVision](https://github.com/Srecharan/GenVision)